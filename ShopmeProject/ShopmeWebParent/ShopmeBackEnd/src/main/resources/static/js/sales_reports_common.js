...
function validateDateRange(reportType, callbackFunction){
	...
	if(days >=7 && days <= 30){
		...
		startDateField.setCustomValidity("Dates must be in the range of 7....30");
...
function getDenominator(period, reportType){
	...
	if(period == "last_7_days") return 28;
...
function setSalesAmount(period,reportType, labelTotalItems){
	...
	denaominator = getDenominator(period, reportType);
	$("#textAvgGrossSales" + reportType).text(formatCurrency(totalGrossSales / denaominator));
	$("#textAvgNetSales" + reportType).text(formatCurrency(totalNetSales / denaominator));
